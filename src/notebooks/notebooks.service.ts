...
export class NotebooksService {
    ...
    try {
      ...
      if (error.code === 'P2025') {
        throw new NotFoundException(
          `Cannot update notebook. Notebook with ID ${id} not found`,
        );
      }
  ...
  async remove(id: string): Promise<Notebook> {
    try {
      ...
      if (error.code === 'P2025') {
        throw new NotFoundException(
          `Cannot delete notebook. Notebook with ID ${id} not found`,
        );
      }
  ...
  async updateStatus(
    id: string,
    statusDto: UpdateNotebookStatusDto,
  ): Promise<Notebook> {
    try {
      return await this.prisma.notebook.update({
        where: { id },
      ...
      throw new InternalServerErrorException(
        'Failed to update notebook status',
      );
    }
  }
